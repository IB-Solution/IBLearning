###LORSQU4IL N'Y A PAS DE COUCHE CACHEE, LE RESEAU FONCTIONNE MAIS PAS QUAND IL Y EN A
...
NEURON_HIDDEN_CELL = 3
# Probablistic Hidden Cell
# Spiking Hidden Cell

NEURON_OUTPUT_CELL = 6
...
class ConvolutionalNeuralNetwork:
    __nbInputNeuron: int = None                                         #Nombre de neurones d'entrée

    __nbHiddenLayer: int = None                                         #Nombre de couche caché
    __hiddenLayerNbNeuron: np.ndarray = None                            #Liste du nombre de neurone de chaque couche
    __hiddenLayerActivationFunction: np.ndarray = None                  #Liste des fonction d'activation de chaque couche (l'index correspond a la position de la couche)
    __hiddenLayerTypeList: np.ndarray = None                            #Liste des type de neurones de chaque couche
    __hiddenLayerWeightList: np.ndarray = None                          #Liste des poids entre chaque neurone [ input->layer1[ hiddenNEuron1[inputNeuron1, inputNeuron2, ...], hiddenNeuron[inputNeuron1, inputNeuron2, ...]], layer1->layer2[[]]]
    __hiddenLayerBiais: np.ndarray = None                               #Liste des taux d'activation de chaque neurone de chaque couche
    __hiddenLayerAlpha: np.ndarray = None                               #Liste de l'alpha de chaque neurone de chaque couche
    __hiddenLayerValueList: np.ndarray = None                           #Liste des valeurs des couches cachées
    __hiddenLayerCoutList: np.ndarray = None                            #Liste des taux d'erreur des couches cachées
    __hiddenLayerDeltaList: np.ndarray = None                           #Liste des importances d'erreur des couches cachées

    __nbOutputNeuron: int = None                                        #Nombre de neurone de sortie
    __outputActivationFunction: int = None                              #Fonction d'activation de la sortie
    __outputLayerType: int = None                                       #Liste de neurone de sortie
    __outputLayerWeightList: np.ndarray = None                          #Liste des poids d'entrées
    __outputLayerBiais: np.ndarray = None                               #Liste des taux d'activation de chaque neurone
    __outputLayerAlpha: np.ndarray = None                               #Liste de l'alpha de chaque neurone
    __outputLayerValue: np.ndarray = None                               #Liste des valeurs des neurones
    __outputLayerCout: np.ndarray = None                                #Liste des taux d'erreur de chaque neurones
    __outputLayerDelta: np.ndarray = None                               #Liste des importances d'erreur de chaque neurone

    __nbMaxNeuronPerLayer: int = None                                   #Nombre de neurone maximal par couche
    ...
    def __init__(self, nbInputNeuron: int, nbOutputNeuron: int, outputNeuronType: int, outputActivationFunction: int, nbMaxNeuronPerLayer: int = 90) -> None:
        """
            ...
            Params:
                ...
                outputNeuronType : Type de neurone de sortie (NEURON_####)
    ...
    def AddLayer(self, nbNeuron: int, neuronType: int, activationFunction: int) -> None:
        ...
        Params:
            ...
            neuronType : Type de neurone de la couche (NEURON_####)
        ...
        self.__hiddenLayerWeightList.resize((self.__nbHiddenLayer+1, self.__nbMaxNeuronPerLayer, self.__nbMaxNeuronPerLayer), refcheck=False)#On ajoute une couche en pluss
    ...
    def SaveNetwork(self, filename: str) -> None:
        ...
        for xLayer in range(self.__nbHiddenLayer):                              #Pour chaque neurone de la liste
            ...
            for xNeuron in range(self.__nbMaxNeuronPerLayer):                       #Pour chaque neurone
                ...
                for inNeuron in range(self.__nbMaxNeuronPerLayer):                      #Pour chaque neurone d'entrée
                    output["hiddenLayers"]["weightList"][xLayer][xNeuron].append(float(self.__hiddenLayerWeightList[xLayer][xNeuron][inNeuron]))
        ...
        for xNeuron in range(self.__nbMaxNeuronPerLayer):                       #Pour chaque neurone
            ...
            for inNeuron in range(self.__nbMaxNeuronPerLayer):                      #Pour chaque neurone d'entrée
                output["outputLayer"]["weightList"][xNeuron].append(float(self.__outputLayerWeightList[xNeuron][inNeuron]))
    ...
    def LoadNetwork(self, filename: str) -> None:
        ...
        for xLayer in range(int(inputData["nbHiddenLayer"])):
            #Ajout d'une couche
            ...
            for xNeuron in range(inputData["nbMaxNeuronPerLayer"]):             #Pour chaque neurone de la couche
                ...
                #Biais
                self.__hiddenLayerBiais[xLayer][xNeuron] = inputData["hiddenLayers"]["biais"][xLayer][xNeuron]

                #Alpha
                self.__hiddenLayerAlpha[xLayer][xNeuron] = inputData["hiddenLayers"]["alpha"][xLayer][xNeuron]
    ...
    def Train(self, dataset: list, expected: list, nbEpoch: int, learningRate: float, acceleration: bool = False, display: bool = False) -> None:
        """
        ...
        if acceleration:
            ...
            self.__hiddenLayerAlpha, self.__hiddenLayerWeightList, self.__hiddenLayerBiais, self.__outputLayerAlpha, self.__outputLayerWeightList, self.__outputLayerBiais = TrainGPU(
                dataset,
                expected,
                nbEpoch,
                learningRate,
            ...
            for epoch in range(nbEpoch):
                cout = 0
                for xInput in range(len(dataset)):
                    self.__FeedForward(dataset[xInput])
                    self.__BackPropagation(expected[xInput])
                    self.__Correction(dataset[xInput], learningRate)
                    cout += np.mean(self.__outputLayerCout[:self.__nbOutputNeuron])
                if display: print("#", epoch, "/", nbEpoch, " Cout =>", cout)
    ...
    def __FeedForward(self, inputs: np.ndarray) -> None:
        ...
        for xHiddenLayer in range(self.__nbHiddenLayer):                                                        #Pour chaque couche cachée
            if self.__hiddenLayerTypeList[xHiddenLayer] == NEURON_HIDDEN_CELL:                                      #Si c'est des neurones cachée
        ...
        #OUTPUT LAYER
        if self.__outputLayerType == NEURON_OUTPUT_CELL:                                                        #Si c'est des neurones de sortie
    ...
    def __BackPropagation(self, expected: np.ndarray) -> None:
        ...
        """        
        if self.__outputLayerType  == NEURON_OUTPUT_CELL:                                                       #Si les neurones sont des neurones de sortie
        ...
        for xLayer in reversed(range(self.__nbHiddenLayer)):                                                    #Pour chaque couche en partant de la fin
            if self.__hiddenLayerTypeList[xLayer] == NEURON_HIDDEN_CELL:                                            #Si la couche sont des neurones cachée      
    ...
    def __Correction(self, inputList: np.ndarray, learningRate: float) -> None:
        ...
        for xLayer in range(self.__nbHiddenLayer):                                                              #Pour chaque couche du reseau
            if self.__hiddenLayerTypeList[xLayer] == NEURON_HIDDEN_CELL:                                            #Si la couche sont des neurones cachées
                if xLayer == 0:                                                                                         #Si c'est la premiere couche
                    ...
                    for xNeuron in range(self.__hiddenLayerNbNeuron[xLayer]):                                               #Pour chaque neurones de la couche
                        ...
                        self.__hiddenLayerBiais[xLayer][xNeuron] += learningRate * self.__hiddenLayerDeltaList[xLayer][xNeuron] #Modification du biais
        #OUTPUT LAYER
        if self.__outputLayerType == NEURON_OUTPUT_CELL:                                                        #Si la couche de sortie sont des neurones de sortie
    ...
    for epoch in range(nbEpoch):                                                                        #Pour chaque entrainement
        for xInput in range(len(dataset)):                                                                  #Pour chaque entrées
            ...
            for xLayer in range(nbHiddenLayer):                                                             #Pour chaque couche caché
                if hiddenListType[xLayer] == NEURON_HIDDEN_CELL:                                                #Si les nerones de la couche sont des neurones caché
                    if xLayer == 0:                                                                                 #Si premiere couche
                        ...
                        # hiddenLayerValueList[xLayer] = np.dot(dataset[xInput], hiddenLayerWeightList[xLayer])               #Multiplication des poids d'entrées et leurs valeurs
                    else:                                                                                               #Sinon
                        for xNeuron in range(hiddenListNbNeuron[xLayer]):                                               #Pour chaque neurone de la couche
                            hiddenLayerValueList[xLayer][xNeuron] = 0                                                       #Reset de la valeur
                        ...
                        # hiddenLayerValueList[xLayer] = np.dot(hiddenLayerValueList[xLayer-1], hiddenLayerWeightList[xLayer])#Multiplication des poids d'entrées et leurs valeurs
            ...
            #Output layer
            if outputLayerType == NEURON_OUTPUT_CELL:                                                           #Si les neurones de la couche de sortie sont des neurones de sortie
                if nbHiddenLayer == 0:                                                                              #Si il n'y a pas de couche caché
                    ...
                    # outputLayerValue = np.dot(dataset[xInput], outputLayerWeightList)                                   #Pré activation
                else:                                                                                               #Sinon
                    for xNeuron in range(nbOutputNeuron):                                                               #Pour chaque neurone de sortie
                        outputLayerValue[xNeuron] = 0                                                                       #Reset du neurone
                    ...
                    # outputLayerValue = np.dot(hiddenLayerValueList[-1], outputLayerWeightList)                          #Multiplication par les poids
            ...
            #---BACK_PROPAGATION---#
            #Output layer
            if outputLayerType == NEURON_OUTPUT_CELL:                                                           #Si le neurones de la couche de sortie sont des neurones de sortie
            ...
            for xLayer in range(nbHiddenLayer-1, -1, -1):                                                       #Pour chaque couche en partant de la fin
                if hiddenListType[xLayer] == NEURON_HIDDEN_CELL:                                                    #Si la couche actuel est de type caché
                    if xLayer == nbHiddenLayer-1:                                                                       #Si la couche actuel est la derniere caché
                        ...
                        # hiddenLayerCoutList[xLayer] = np.dot(outputLayerDelta, outputLayerWeightList.T)                     #Calcul du taux d'erreur de chaque liaison
                    else:                                                                                               #Sinon
                        for xNeuron in range(hiddenListNbNeuron[xLayer]):                                                   #Pour chaque neurone de la couche
                            hiddenLayerCoutList[xLayer][xNeuron] = 0                                                            #Reset du taux d'erreur
                        ...
                        # hiddenLayerCoutList[xLayer] = np.dot(hiddenLayerDeltaList[xLayer+1], hiddenLayerWeightList[xLayer+1].T)#Calcul du taux d'erreur de chaque liaison
            ...
            #Output layer
            if outputLayerType == NEURON_OUTPUT_CELL:                                                           #Si la couche de sortie est de type output
